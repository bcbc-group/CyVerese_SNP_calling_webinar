...
setwd("/Users/jacoblandis/Desktop/Upload_for_webinar/test")

#call VCF file
vcf.fn <- "Ugibba_LD_pruned_SNPs.recode.vcf"
...
#LD prune
set.seed(1000)
snpset <- snpgdsLDpruning(genofile, ld.threshold=1, autosome.only=FALSE, slide.max.n=10000,start.pos="random")

names(snpset)
snpset.id <- unlist(snpset)

#run PCA
pca <- snpgdsPCA(genofile, snp.id=snpset.id,autosome.only=FALSE, num.thread=2)
